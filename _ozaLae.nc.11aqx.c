/*
 *	11aqx/3e3:johndavidjones:vnhavasa:::
...
int main(int argc, char* argv[]){
  ...
  /* setlocale(LC_ALL, "he_IL");*/
   ...
   upoLa = fopen(ukLoa, "w");
	...
	uxaaL[gia]	= eKa[0];
	uxama[gia]      = eKa[0];
	if((uxama[gia] == L'כ') && ugkuma[gia] == 0 ){
	    uxama[gia] = L'ך';
	}
	if((uxama[gia] == L'מ') && ugkuma[gia] == 0 ){
	    uxama[gia] = L'ם';
	}
	if((uxama[gia] == L'נ') && ugkuma[gia] == 0 ){
	    uxama[gia] = L'ן';
	}
	if((uxama[gia] == L'פ') && ugkuma[gia] == 0 ){
	    uxama[gia] = L'ף';
	}
	if((uxama[gia] == L'צ') && ugkuma[gia] == 0 ){
	    uxama[gia] = L'ץ';
	}
	ckT.chars[0] = uxama[gia];
	add_wch(&ckT);
	egkoLa[0]	= boL[ugkuLa[gia]];
	if((egkoLa[0] == 'k') && (ugkuma[gia] == 0)){
	    egkoLa[0] = 'K';
	}
	if((egkoLa[0] == 'm') && (ugkuma[gia] == 0)){
	    egkoLa[0] = 'M';
	}
	if((egkoLa[0] == 'n') && (ugkuma[gia] == 0)){
	    egkoLa[0] = 'N';
	}
	if((egkoLa[0] == 'p') && (ugkuma[gia] == 0)){
	    egkoLa[0] = 'P';
	}
	if((egkoLa[0] == 'x') && (ugkuma[gia] == 0)){
	    egkoLa[0] = 'X';
	}
	uotaaL[gia]     = egkoLa[0];
	ugkaaL[gia]	= egkoLa[0];
...
void dot(p* upeL){
	...
	case 'L':
	    ...
	    La(upeL->bbaL, uxama[Ta(ka(upeL->ox, 2), oLuotaL)]);
	    sT(upeL->bbaL);
	    eo[0]	= 0;
	    eo[1]	= 100;
	    while(eo[0] < eo[1]){
		...
		La(upeL->bbaL, uxama[Ta(ka(ka(upeL->ox, 2), eo[0]), oLuotaL)]);
		sT(upeL->bbaL);
	        eo[0]	= ka(eo[0], 1);
	    }
	    break;
